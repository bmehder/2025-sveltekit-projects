<script>
	...
	// Component Props
	let { title = 'Todo List', items = [] } = $props()
	...
	const addTodo = newTodo =>
		list.push({ name: newTodo, isDone: false, id: crypto.randomUUID() })

	const updateTodo = index => (list[index].isDone = !list[index].isDone)
...
<div class="todo-list">
	...
	<ul>
		{#each list as item, index (item.id)}
			<li transition:slide>
				<button class:line-through={item.isDone} onclick={() => handleUpdate(index)}
					>{item.name}</button
				>
